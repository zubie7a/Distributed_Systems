...
// provided by the teacher, which were C methods for formatting, reading, etc.
    ...
    // Code for the Master node
        ...
        lineNum = 0;
        char* reading = new char [71];
        while(fscanf(in,"%s", reading) != EOF) {
            char* line = new char [71];
            strcpy(line, reading);
            int CUR_PROC = lineNum % NUM_PROCS;
            stringLists[CUR_PROC].push_back(line);
            lineNum++;
        }
            ...
            for(int CUR_ID = 0; CUR_ID < NUM_PROCS; CUR_ID++) {
                if(iter < stringLists[CUR_ID].size()) {
                    ...
                    if(CUR_ID == 0) {
                        ...
                        MPI_Send(str, 71, MPI_CHAR, CUR_ID, FROM_MASTER,
                                 MPI_COMM_WORLD);
                    }
                }
            }
    ...
    // Code for the Worker nodes
        ...
        // may receive the same amount of strings to process.
            char *str = new char [71];
            MPI_Recv(str, 71, MPI_CHAR, MASTER, FROM_MASTER,
                     MPI_COMM_WORLD, &status);
            stringList.push_back(str);
        }
    }
    ...
    // process which is then modified with the values and returned at the end.
        char *text = stringList[j];
        char *sp1 = new char[71];
        char *sp2 = new char[71];
        char op;
        int i = 1;
        while (text[i] != '=') {
        ...
        double val1 = (double)atoi(sp1);
        double val2 = (double)atoi(sp2);
        double result = -1;
        if (op == '+')
            result = val1 + val2;
        else if (op == '-')
            ...
            result = val1 / (val2 + 1);
        sprintf(stringList[j], "%.0lf %c %.0lf = %.2lf\n", val1, op, val2, result);
    }
    ...
    // Code for the Master node.
        ...
        for(int i = 0; i < REMAINING; i++) {
            char *str = new char [71];
            MPI_Recv(str, 71, MPI::CHAR, MPI_ANY_SOURCE, FROM_WORKER,
                     MPI_COMM_WORLD, &status);
        ...
        for(int i = 0; i < stringList.size(); i++) {
            MPI_Send(stringList[i], 71, MPI::CHAR, MASTER, FROM_WORKER,
                     MPI_COMM_WORLD);
        }
        printf("TASK_ID: %d, Total Time: %lf\n", TASK_ID, MPI_Wtime() - t1);
    }
